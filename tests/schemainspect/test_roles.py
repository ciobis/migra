...
def test_roles(db):
    with S(db) as s:
        ...
        assert (admin_role.create_statement == 'create role admin with NOSUPERUSER NOCREATEDB INHERIT NOLOGIN NOREPLICATION NOBYPASSRLS connection limit -1 password NULL ;')
        assert (admin_role.update_statement == 'alter role admin with NOSUPERUSER NOCREATEDB INHERIT NOLOGIN NOREPLICATION NOBYPASSRLS connection limit -1 password NULL ;')
